...
#     print("Evaluation on : %s " % (val_setname))
    ...
    data=sio.loadmat('../../../data/SIDD_test_existing_estimation_255.mat')
...
#         print("Avg psnr_dbsn: %.4f" % (psnr_val))
    ...
    with torch.no_grad():
        ...
        for i in range(n):
            ...
            Inoisy = torch.from_numpy(data['noisy_images'][i] / 255.).reshape(1,1,H,W).to(torch.float32)

    #         Idenoised_crop_c = denoiser(Inoisy)

            img_noise_val = Inoisy.cuda()
            _,C,H,W = img_noise_val.shape
            # forward backbone
            mu_out_val, mid_out_val = dbsn_model(img_noise_val)
            # forward sigma_mu
            ...
            denoised_arr.append(map_out_val[0,0].detach().cpu().numpy())
            denoised_mu_arr.append(mu_out_val[0,0].detach().cpu().numpy())

    sio.savemat('../../../result_data/201104_DBSN_xhat_SIDD_benchmark.mat', {'denoised_arr': denoised_arr})
    sio.savemat('../../../result_data/201104_DBSN_mu_SIDD_benchmark.mat', {'denoised_arr': denoised_mu_arr})
